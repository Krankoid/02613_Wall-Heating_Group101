...
def jacobi(u, interior_mask, max_iter, atol=1e-6):
    # Optimized port: same structure as the naive jacobi in tasks/task9, with
    # boolean fancy indexing replaced by cp.where to eliminate per-iteration
    # cupy_scan_naive / cupy_getitem_mask kernels and DtoH transfers.
    u = cp.copy(u)

    for i in range(max_iter):
        u_new = 0.25 * (u[1:-1, :-2] + u[1:-1, 2:] + u[:-2, 1:-1] + u[2:, 1:-1])
        delta = cp.where(interior_mask, cp.abs(u[1:-1, 1:-1] - u_new), 0).max()
